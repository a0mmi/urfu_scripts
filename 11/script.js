(function () {
    // По сути ненужная проверка на то, что бинарная строка или нет
    function isBinaryString(s) {
        return /^[01]+$/.test(s);
    }

    // Сколько контрольных битов r нужно для k информационных:
    // 2^r >= k + r + 1
    ...
    // ---- Алгоритмы ----
    // Кодирование: принимает информационные биты слева->направо (строка),
    // возвращает объект с закодированной строкой и деталями.
    function encode(dataStr) {
        ...
        // bits — 1-based массив (pos: 1..n)
        var bits = new Array(n + 1);
        for (var i = 0; i <= n; i++) bits[i] = 0;

        // Заполняем информационные биты (справа — позиция 1)
        var di = k - 1;
        ...
        if (!isPowerOfTwo(pos)) {
            ...
            bits[pos] = 0; // контрольные биты пока 0
        }
        }

        // Вычисляем контрольные биты (паритеты)
        for (var p = 0; p < r; p++) {
        var parityPos = 1 << p;
        var parity = 0;
        for (var pos = 1; pos <= n; pos++) {
        ...
        // Собираем результат в строку (с позиции n слева к 1 справа)
        var out = "";
  ...
  // Декодирование: принимает закодированную строку и возвращает
  // объект с результатом, синдромом и (возможной) коррекцией.
    function decode(encodedStr) {
    if (!isBinaryString(encodedStr)) return { success: false, message: "Encoded input must be binary (only 0 and 1).", data: null };
    ...
    // Вычисляем синдром — позиция ошибочного бита (в двоичной маске)
    var syndrome = 0;
    for (var p = 0; p < r; p++) {
      var parityPos = 1 << p;
      var parity = 0;
    ...
    if (syndrome !== 0) {
      if (syndrome <= n) {
        // Переворачиваем бит в обнаруженной позиции
        bits[syndrome] = bits[syndrome] ^ 1;
        corrected = true;
        correctedPos = syndrome;
      } else {
        // Синдром указывает за пределы длины — вероятно несколько ошибок
        return {
          success: false,
    ...
    // Извлекаем информационные биты (в порядке слева->направо)
    var dataBits = [];
    ...
    return { success: true, data: dataStr, corrected: corrected, message: message, correctedPos: correctedPos, syndrome: syndrome, bitsArray: bits, n: n, r: r };
  }
  ...
  var radioMode = function () { return document.querySelector('input[name="mode"]:checked').value; };

  // Простая функция вывода в блок
  function show(text) {
    outputPre.textContent = text;
  }

  // Форматируем строки позиций и битов (pos n ... 1) — для читабельности
  ...
  // Инициалный пример (комментирован)
  // show("Результат появится здесь...");
})();
