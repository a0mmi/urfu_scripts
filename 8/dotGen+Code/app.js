...
// Возвращает строку DOT для Graphviz. Учитывает метки 0/1 (левый=0, правый=1).
function generateDOT(root) {
  if (!root) return "digraph G { }";
  var lines = [];
  var idCounter = 0;
  ...
  function assignIds(node) {
    ...
    if (node.child) for (var i=0;i<node.child.length;i++) assignIds(node.child[i]);
  }
  assignIds(root);

  function emitNodes(node) {
    var label = escLabel(node.str) + " (" + node.freq + ")";
    lines.push('  ' + node._id + ' [label="' + label + '"];');
    if (node.child) {
      for (var i=0;i<node.child.length;i++) {
        emitNodes(node.child[i]);
      }
    }
  }
  emitNodes(root);

  function emitEdges(node) {
    if (!node.child) return;
    for (var i=0;i<node.child.length;i++) {
      var ch = node.child[i];
      var edgeLabel = (i === 0) ? "0" : "1";
      lines.push('  ' + node._id + ' -> ' + ch._id + ' [label="' + edgeLabel + '"];');
      emitEdges(ch);
    }
  }
  emitEdges(root);

  var dot = "digraph Huffman {\n  node [shape=box,fontsize=10];\n" + lines.join("\n") + "\n}\n";
  return dot;
}
...
(function main() {
    ...
    var dot = generateDOT(root);
    var dotPath = scriptFolder + "tree.dot";
    var ts = fso.CreateTextFile(dotPath, true);
    ts.Write(dot);
    ts.Close();
    WScript.Echo("DOT written to: " + dotPath);
})();